chrome.runtime.onMessageExternal.addListener((request, sender, sendResponse) => {
    if (request.type === "GET_GOOGLE_COOKIES") {
        chrome.cookies.getAll({ domain: ".google.com" }, (cookies) => {
            ...
            if (filteredCookies.length > 0) {
                sendResponse({ success: true, cookies: filteredCookies.map(c => `${c.name}=${c.value}`).join("; ") });
            } else {
                sendResponse({ success: false, error: "Không tìm thấy Cookie." });
            }
        });
        return true;
    }
    ...
    if (request.type === "RUN_GOOGLE_API") {
        executeApiInFlowTab(request.payload).then(sendResponse);
        return true; 
    }

    if (request.type === "POLL_GOOGLE_API") {
        executePollInFlowTab(request.payload).then(sendResponse);
        return true;
    }
});
            ...
            func: async (requestData) => {
                try {
                    ...
                    const clientContext = {
                        ...
                        projectId: PROJECT_ID, tool: "PINHOLE", sessionId: ";" + Date.now()
                    };
